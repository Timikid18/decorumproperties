...
export function WelcomePopup() {
  ...
  return (
    ...
    >
      ...
      {/* card */}
      <div
        className={`relative z-10 w-full max-w-md overflow-hidden rounded-2xl bg-surface shadow-2xl transition-all duration-[400ms] ease-out ${closing ? "scale-95 opacity-0" : "scale-100 opacity-100"}`}
      >
        {/* decorative header */}
        <div className="relative bg-brand-800 px-6 pt-8 pb-14 text-center text-white">
          <p className="inline-flex items-center gap-2 rounded-full bg-surface/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide">
          ...
          <div className="absolute -bottom-6 left-1/2 flex h-12 w-12 -translate-x-1/2 items-center justify-center rounded-full bg-surface shadow-md ring-4 ring-white">
        ...
        <button
          onClick={dismiss}
          aria-label="Close"
          className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-surface/15 text-white backdrop-blur transition-colors hover:bg-surface/30"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
